...
const thinTeal  = { style: BorderStyle.SINGLE, size: 6,  color: C.teal };
...
function sp(b = 80) { return new Paragraph({ children: [], spacing: { before: b, after: 0 } }); }
...
function run(text: string, opts: { bold?: boolean; italic?: boolean; size?: number; color?: string } = {}) {
  return new TextRun({ text, font: 'Calibri', size: opts.size ?? 20, color: opts.color ?? C.mid, bold: opts.bold, italics: opts.italic });
}

function para(children: TextRun[], spacing = { after: 80 }, align?: typeof AlignmentType[keyof typeof AlignmentType]) {
...
export async function generateResumeDocx(markdown: string, name: string): Promise<Buffer> {
  ...
  const leftLines: string[] = [];   // Skills, Education, NYSC
  const rightLines: string[] = [];  // Profile, Experience, Projects

  let currentSection = 'header';
  for (const line of lines) {
    if (line.startsWith('# ')) { headerLines.push(line); continue; }
    const low = line.toLowerCase();
    if (low.startsWith('## ')) {
      if (low.includes('skill') || low.includes('education') || low.includes('nysc') || low.includes('cert')) currentSection = 'left';
      else currentSection = 'right';
    }
    if (currentSection === 'header') headerLines.push(line);
    else if (currentSection === 'left') leftLines.push(line);
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      new TableCell({
        ...
        borders: noBorders, width: { size: 10306, type: WidthType.DXA },
        margins: { top: 200, bottom: 200, left: 300, right: 300 },
        children: [
          ...lines.filter(l => l.startsWith('# ')).map(l =>
            new Paragraph({ children: [run(l.slice(2), { bold: true, size: 44, color: C.white })], spacing: { after: 60 } })
          ),
          new Paragraph({ children: [run(name, { size: 19, color: 'A8C8FF', italic: true })], spacing: { after: 0 } }),
        ],
      }),
    ]}),
  ]});
  ...
  const bodyTable = new Table({
    ...
    rows: [new TableRow({ children: [
      new TableCell({ borders: noBorders, width: { size: 3400, type: WidthType.DXA }, margins: { top: 0, bottom: 0, left: 0, right: 200 }, children: leftChildren }),
      new TableCell({ borders: { top: none, bottom: none, left: { style: BorderStyle.SINGLE, size: 6, color: C.line }, right: none }, width: { size: 160, type: WidthType.DXA }, children: [sp()] }),
      new TableCell({ borders: noBorders, width: { size: 6746, type: WidthType.DXA }, margins: { top: 0, bottom: 0, left: 200, right: 0 }, children: rightChildren }),
    ]}),
  ]});
      ...
      children: [
        ...
        ...paragraphs.map((p, i) => {
          ...
          if (p.startsWith('Yours') || p.startsWith('Dear')) {
            return para([run(p, { bold: p.startsWith('Dear') || p.startsWith('Yours'), size: 20, color: C.dark })], { before: p.startsWith('Yours') ? 200 : 0, after: p.startsWith('Dear') ? 160 : 80 });
          }
          if (p.startsWith('Re:')) {
            return para([run(p, { bold: true, size: 20, color: C.navy })], { after: 200 });
          }
          // Name sign-off at end (after Yours faithfully)
          if (i > paragraphs.length - 5 && !p.includes(' ') === false && p === name) {
...
export async function generateEmailTemplatesDocx(
  ...
  for (const tmpl of templates) {
    children.push(
      ...
      para([run('SUBJECT LINE:', { bold: true, size: 19, color: C.teal }), run(`  ${tmpl.subject}`, { size: 19, color: C.dark })], { after: 120 }),
    );
    for (const line of tmpl.body.split('\n')) {
      if (!line.trim()) { children.push(sp(60)); continue; }
